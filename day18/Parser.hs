...
happyExpList = Happy_Data_Array.listArray (0,15) ([32800,4608,0,512,18432,64,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_snailfish","Snail","Snail1","'['","']'","','","num","%eof"]
...
happyReduction_3 (HappyTerminal (Number happy_var_1))
	 =  HappyAbsSyn5
		 (Leaf happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	RightBra -> cont 7;
	Comma -> cont 8;
	Number happy_dollar_dollar -> cont 9;
	_ -> happyError' ((tk:tks), [])
	}
...
snailfish tks = happyRunIdentity happySomeParser where
